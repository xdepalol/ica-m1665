var myTheme = {
	...
	common : {
		...
		init : function(){
			...
			// External links
			$(".iDevice a[target='_blank']").each(function(){
				var e = $(this);
				var ref = e.attr("href");
				if (typeof(ref)!='string') return;
				if (ref.indexOf("http")!=0&&ref.indexOf("//")!=0) return;
				if (this.rel&&typeof(this.rel)=='string'&&this.rel.indexOf("lightbox")!=-1) return;
				var h = e.html();
					// Allow EM
					h = h.replace("<em>","");
					h = h.replace("</em>","");
					...
					h = h.replace("<strong>","");
					h = h.replace("</strong>","");
					// Allow SPAN (might have attributes)
					var tmp = $("<div>"+h+"</div>");
					$("span",tmp).each(function(){
						var x = $(this)
						var xT = x.text();
						x.before(xT);
						x.remove();
